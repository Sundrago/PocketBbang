...
public class DangunControl : MonoBehaviour
{
    ...
    private void Start()
    {
        print(nowTime = System.DateTime.Now.ToString(format));
    ...
    public void UpdateState()
    {
        ...
        if (available[5])
        {
            if(PlayerPrefs.GetInt("sellUpdated" + 0) == 0) ResetSellCard(0);
        }

        if (available[6])
        {
            if (PlayerPrefs.GetInt("sellUpdated" + 1) == 0) ResetBuyCard(1);
        }
    ...
    public bool CheckDateAvailability(int idx)
    {
        ...
        if (!System.DateTime.TryParseExact(PlayerPrefs.GetString("dangun" + idx), format, provider, System.Globalization.DateTimeStyles.None, out oldTime))
        {
            PlayerPrefs.SetString("dangun" + idx, "2022/06/15 04:48:15");
            if (!System.DateTime.TryParseExact(PlayerPrefs.GetString("dangun" + idx), format, provider, System.Globalization.DateTimeStyles.None, out oldTime))
            {
                balloon.ShowMsg("데이터 오류 : dangun" + idx);
            }
        }
    ...
    public string GetRemainTime(int idx)
    {
        ...
        timeGap = oldTime.AddSeconds(interval) - System.DateTime.Now;

        string outputString = timeGap.Minutes + "분 " + timeGap.Seconds + "초";
        return outputString;
    }
    ...
    public bool NeedToUpdate()
    {
        for(int i = 0; i< available.Length; i++)
        {
            if (available[i] == false) return true;
        }
        return false;
    }
    ...
    public void SetDateData(int idx) {
        nowTime = System.DateTime.Now.ToString(format);
        ...
        switch(idx)
        {
            ...
            case 5:
                Msg.SetMsg(collection.myCard[PlayerPrefs.GetInt("sellIdx" + 0)].rank + "급 스티커\n" +
                    ...
                    "\n(나의 보유 슈량 : " + collection.myCard[PlayerPrefs.GetInt("sellIdx" + 0)].count + "개)", 2, "BuyCardCallBack");
                break;
            case 6:
                Msg.SetMsg(collection.myCard[PlayerPrefs.GetInt("sellIdx" + 1)].rank + "급 스티커\n" +
                    ...
                    "\n(나의 보유 슈량 : " + collection.myCard[PlayerPrefs.GetInt("sellIdx" + 1)].count + "개)", 2, "SellCardCallBack");
                break;
            default:
