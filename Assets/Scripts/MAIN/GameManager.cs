...
public class GameManager : MonoBehaviour
{
    ...
    [Header("Managers and Controllers")] [FormerlySerializedAs("myAudio")]
    public AudioController audioController;

    [FormerlySerializedAs("showroom")] public BbangShowroomManager showroomManager;
    [FormerlySerializedAs("balloon")] public BalloonUIManager balloonUIManager;
    [FormerlySerializedAs("alba")] public AlbaMinigameManager albaMinigameManager;
    [FormerlySerializedAs("Msg")] public PhoneMessageController phoneMessageController;
    [FormerlySerializedAs("setAudio")] public SettingPanelManager settingPanelManager;
    [FormerlySerializedAs("dangunCha")] public DangunCharacterManager dangunCharacterManager;
    [FormerlySerializedAs("achievement")] public AchievementManager achievementManager;

    [FormerlySerializedAs("collectionPanel")]
    public CollectionPanelManager collectionPanelManager;

    [FormerlySerializedAs("notice")] public DangunNotificationManager dangunNotificationManager;
    [FormerlySerializedAs("rank")] public RankManager rankManager;
    [FormerlySerializedAs("dduk")] public DdukMinigameManager ddukMinigameManager;

    [FormerlySerializedAs("domiCoin")] [SerializeField]
    private DoMiCoinManager doMiCoinManager;
    ...
    [FormerlySerializedAs("albabPhoneManager")] [SerializeField]
    private AlbaPhoneManager albaPhoneManager;
    ...
    public bool albaMode;
    public PrompterController pmtComtrol;
    ...
    private void Start()
    {
        ...
        pmtComtrol = prompter.GetComponent<PrompterController>();
        ...
        goToStoreBtn.GetComponent<Image>().color =
            PlayerPrefs.GetInt("totalBbangCount", 0) == 0 ? Color.yellow : Color.white;

        if (PlayerPrefs.GetInt("debugMode") == 1) SetDebugMode();

        rankManager.Start();
        dangunNotificationManager.Start();
    }
    ...
    private void Update()
    {
        ...
        if (Time.frameCount % 30 == 0)
            if (callBbangboy & (currentLocation == "home"))
            {
                ...
                {
                    case 0:
                        balloonUIManager.ShowMsg("똑똑- 노크소리가 들린다.");
                        gameObject.GetComponent<BbangBoyContoller>().Boy_In();
                        break;
                    case 1:
                        balloonUIManager.ShowMsg("똑똑- 노크소리가 들린다.");
                        dangunCharacterManager.PlayDangunCharacterAnimation(callIdx);
                        break;
                    case 2:
                        balloonUIManager.ShowMsg("똑똑- 노크소리가 들린다.");
                        dangunCharacterManager.PlayDangunCharacterAnimation(callIdx);
    ...
    private void InitPlayerPrefs()
    {
        ...
        if (!PlayerPrefs.HasKey("ChaName") | !PlayerPrefs.HasKey("ChaIdx"))
        {
            ...
            PlayerPrefs.Save();
            chaSelectPanel.GetComponent<CharacterSelector>().OpenChaSelect();
        }
    ...
    public void DebugBtn()
    {
        debugCount += 1;
        if (debugCount >= 15)
            phoneMessageController.SetMsg("디버그 모드에 진입하면 영구적으로 기록이 남으며 랭킹 시스템에서 제외됩니다.\n디버그 모드에 진입하시겠습니까?", 2, "debug");
    }
    ...
    {
        debugPanel.SetActive(false);
        collection.GetComponent<CollectionPanelManager>().debugMode = false;
        debugCount = 0;
    }

    public void SetCollectionDebugMode()
    {
        collection.GetComponent<CollectionPanelManager>().debugMode = true;
    ...
    public void BtnClicked(string idx)
    {
        ...
        {
            case "collection":
                collection.GetComponent<CollectionPanelManager>().Start();
                collection.GetComponent<CollectionPanelManager>().ShowPanel();
                debugCount = 0;
                break;
            case "store":
                ...
                {
                    if (playerHealthManager.ConsumeSingleHeart())
                    {
                    ...
                    {
                        balloonUIManager.ShowMsg("지금은 피곤하다.. \n 너튜브나 보면서 쉴까..");
                    }
            ...
            case "phone":
                ...
                break;
            case "close_phone":
                albaPhoneManager.gameObject.SetActive(false);
                if (hidePhoneTmp)
                {
            ...
            case "reset":
                ...
                balloonUIManager.ShowMsg("초기화 성공! 앱을 재실행 해주세요.");
                Application.Quit();
                break;
        }
    }
    ...
    public void BbangBoyAction(string action)
    {
        ...
        switch (action)
        {
            ...
            case "CallBbangBoy":
                ...
                {
                    balloonUIManager.ShowMsg("집으로 돌아가자!");
                    callBbangboy = true;
                    callIdx = 0;
                    phone_ui.GetComponent<Animator>().SetTrigger("hide");
                    lower_bar.GetComponent<Animator>().SetTrigger("hide");
    ...
    public void ActionHandler(string action)
    {
        ...
        switch (action)
        {
            ...
            case "store_in":
                ...
                if (myStoreType == 9) audioController.PlayMusic(9);
                else audioController.PlayMusic(2);
                EnterStore();
                break;
            ...
            case "park":
                ...
                achievementManager.Autheticate();
                gointToPark = true;
    ...
    {
        albaPhoneManager.gameObject.SetActive(true);
    }
    ...
    public void GoToAlba()
    {
        if (!playerHealthManager.ConsumeSingleHeart())
        {
            balloonUIManager.ShowMsg("지금은 좀 피곤하다..");
            return;
        }
        ...
        var albaIdx = PlayerPrefs.GetInt("GotoAlbaIdx");
        var albaStoreType = 0;
        var albaString = "";
    ...
    private void EnterStore()
    {
        ...
        if (chaIdx == 0)
        {
            ...
            {
                //timer not yet
                if (dangunNotificationManager.TimeGap() <= 0)
                {
                ...
                else if (dangunNotificationManager.TimeGap() <= 5)
                {
                    //2-3개
                ...
                else if (dangunNotificationManager.TimeGap() <= 60)
                {
                    //1개
                    pmtComtrol.AddString("미소녀", "조금만 더 일찍 오지!");
        ...
        {
            doMiCoinManager.UpdateCurrentPrice();
    ...
    public void StoreEvent(string code)
    {
        ...
        switch (code)
        {
            ...
            case "notiSetting":
#if UNITY_IPHONE
                phoneMessageController.SetMsg("아이폰 설정으로 이동할까요?\n[알림] 탭에서 [포켓볼빵 더게임] 알림 설정을 확인해주세요.", 2,
                    "SendtoSetting");
#endif
            ...
            case "hostf_ball":
                if (!matdongsanBuy)
                {
                    ...
                    pmtComtrol.AddString("점주", "맛동산은 만냥이야. 어떻게 할 텐가?");
            ...
            case "hostf_buy_check":
                phoneMessageController.SetMsg("맛동산 1상자와 포켓볼빵을 10,000냥에 구매합니다.", 2, "matdongsanf");
                break;
            ...
            case "hostf_buy":
                gameObject.GetComponent<PlayerHealthManager>().UpdateMoney(-10000);
                pmtComtrol.Reset();
                pmtComtrol.imageMode = true;
                ...
                if (!playerHealthManager.IsHeartEmpty())
                {
                    pmtComtrol.AddOption("편의점을 나간다.", "store", "hostf_out");
                    pmtComtrol.AddOption("알바를 한다.", "store", "hostf_alba");
                }
            ...
            case "albaf_start":
                playerHealthManager.ConsumeSingleHeart();
                albaMinigameManager.StartAlba(true);
                break;
            ...
            case "host_buy_check":
                phoneMessageController.SetMsg("맛동산 1상자와 포켓볼빵을 10,000에 구매합니다.", 2, "matdongsan");
                break;
    ...
    public void AddBBangType(int amount, string type, string idx = "")
    {
    ...
    {
        dangunNotificationManager.MakeAndSetNotification();
    }
    ...
    public void OpenBbang()
    {
        ...
        //AddBBang(-1);
        if (currentLocation == "bbobgi") newStore.GetComponent<StoreControl>().bbobgiPanel.bbobgiController.HideObjs();

        if (PlayerPrefs.GetInt("new_purin") > 0)
        {
            AddBBangType(-1, "-", "purin");
            var myBang = new GameObject();
            myBang = Instantiate(bbang_panel, bbang_holder.transform);
            myBang.SetActive(true);
            myBang.GetComponent<BbangAnimationBbangSprites>().SetBbang(5);

            showroomManager.AddBbang(BbangShowroomManager.BbangType.bbang_purin, 1);
        }
        else if (PlayerPrefs.GetInt("new_maple") > 0)
        {
            AddBBangType(-1, "-", "maple");
            var myBang = new GameObject();
            myBang = Instantiate(bbang_panel, bbang_holder.transform);
            myBang.SetActive(true);
            myBang.GetComponent<BbangAnimationBbangSprites>().SetBbang(4);

            showroomManager.AddBbang(BbangShowroomManager.BbangType.bbang_maple, 1);
        }
        else if (PlayerPrefs.GetInt("new_bingle") > 0)
        {
            AddBBangType(-1, "-", "bingle");
            var myBang = new GameObject();
            myBang = Instantiate(bbang_panel, bbang_holder.transform);
            myBang.SetActive(true);
            myBang.GetComponent<BbangAnimationBbangSprites>().SetBbang(3);

            showroomManager.AddBbang(BbangShowroomManager.BbangType.bbang_bingle, 1);
        }
        else if (PlayerPrefs.GetInt("new_hot") > 0)
        {
            ...
            myBang = Instantiate(bbang_panel, bbang_holder.transform);
            myBang.SetActive(true);
            myBang.GetComponent<BbangAnimationBbangSprites>().SetBbang(2);

            showroomManager.AddBbang(BbangShowroomManager.BbangType.bbang_hot, 1);
        }
        else if (PlayerPrefs.GetInt("new_strawberry") > 0)
        {
            AddBBangType(-1, "-", "strawberry");
            var myBang = new GameObject();
            myBang = Instantiate(bbang_panel, bbang_holder.transform);
            myBang.SetActive(true);
            myBang.GetComponent<BbangAnimationBbangSprites>().SetBbang(1);

            showroomManager.AddBbang(BbangShowroomManager.BbangType.bbang_strawberry, 1);
        }
        else if (PlayerPrefs.GetInt("new_choco") > 0)
        {
            AddBBangType(-1, "-", "choco");
            var myBang = new GameObject();
            myBang = Instantiate(bbang_panel, bbang_holder.transform);
            myBang.SetActive(true);
            myBang.GetComponent<BbangAnimationBbangSprites>().SetBbang(0);

            showroomManager.AddBbang(BbangShowroomManager.BbangType.bbang_choco, 1);
        }

        originalMusic = audioController.currentPlaying;
        audioController.PlayMusic(3);
    }

    public void BBangClosed()
    {
        audioController.PlayMusic(originalMusic);
    }
    ...
    public void AdsYes()
    {
        if (gameObject.GetComponent<PlayerHealthManager>().IsHeartFull())
        {
            balloonUIManager.ShowMsg("이제 그만 쉬어도 되겠다.");
            return;
        }

        gameObject.GetComponent<ADManager>().PlayAds(ADManager.AdsType.heart);
        watchAdPanel.SetActive(false);
    }
    ...
    public void BusBtnClicked()
    {
        ...
        BusBtnUpdate();
        gameObject.GetComponent<PlayerHealthManager>().Bus_SetAnim();
    }
    ...
    public void BusLaunch()
    {
        ...
        {
            gameObject.GetComponent<PlayerHealthManager>().Bus_Receive();
        }
        else
        {
            gameObject.GetComponent<PlayerHealthManager>().Bus_launch();
            if (PlayerPrefs.HasKey("ShuttleCount"))
            {
    ...
    public void BusBtnUpdate()
    {
        debugCount = 0;
        //gameObject.GetComponent<PlayerHealthManager>().Bus_SetAnim();
        if (gameObject.GetComponent<PlayerHealthManager>().went)
        {
        ...
        {
            if (gameObject.GetComponent<PlayerHealthManager>().received)
            {
                lauchText.text = "빵셔틀 보내기";
    ...
    {
        dangunPanel.GetComponent<DangunManager>().OpenDangun();
    }

    public void CreditBtnClicked()
    {
        collection.GetComponent<CollectionPanelManager>().GetCount();
    ...
    {
        settingPanel.GetComponent<SettingPanelManager>().Start();
    ...
    public void Debug_RestoreDebugData()
    {
        ...
        balloonUIManager.ShowMsg("디버그 기록 삭제");
    }

    public void Debug_AddVacance()
    {
        showroomManager.AddBbang(BbangShowroomManager.BbangType.bbang_vacance, 1);
    }

    public void Debug_AddYogurt()
    {
        showroomManager.AddBbang(BbangShowroomManager.BbangType.bbang_yogurt, 1);
    }
    ...
    public void Debug_MakeCards()
    {
        ...
        balloonUIManager.ShowMsg("카드 15장 됐어요!");
    }
    ...
    {
        phoneMessageController.SetMsg("디버그 모드를 사용한 이력이 있어 랭킹 시스템에서 제외됩니다. 랭킹을 사용하려면 데이터를 초기화 해주세요.", 1);
    ...
    public void Tanghuru(string ID)
    {
        ...
        switch (ID)
        {
            ...
            case "ID_T_OPEN_STORE_TANGHURU_2":
                pmtComtrol.AddString("왕형", "탕후루는 하나에 3,000냥이오. 구매하겠소?");
            ...
            case "ID_T_OPEN_STORE_TANGHURU_BUY_YES":
                ...
                playerHealthManager.UpdateMoney(-3000);
    ...
    private void Nylon_OpenTradePanel(bool flag)
    {
        ...
        if (flag) doMiCoinManager.ShowPanel();
        else doMiCoinManager.HidePanel();
    }
    ...
    public void Nylon(string ID)
    {
        ...
        switch (ID)
        {
            ...
            case "ID_F_BBANG":
                ...
                pmtComtrol.AddString("나이롱마스크", "지금 만 냥을 내면 나중에 포켓볼 뽱으로 돌려줄게요우~");
                pmtComtrol.AddString("훈이", "몇 개 줄 건데요?");
                pmtComtrol.AddString("나이롱마스크", "그때그때 달라요우~");
                pmtComtrol.AddString("훈이", "(왠지 사기꾼 같은데...)");
                Nylon_TryInvestBbang("네, 만 냥 투자할게요.", "ID_F_BBANG_INVEST", "ID_F_NOMONEY_BEFORECOIN");
            ...
            case "ID_F_BBANG_INVEST":
                ...
                playerHealthManager.UpdateMoney(-10000);
            ...
            case "ID_F_CHICKEN_WHAT":
                ...
                pmtComtrol.AddString("나이롱마스크", "지금 만 을 내면 나중에 포켓볼 뽱으로 돌려줄게요우~");
                pmtComtrol.AddString("훈이", "몇 개 줄 건데요?");
                pmtComtrol.AddString("나이롱마스크", "그때그때 달라요우~");
                pmtComtrol.AddString("훈이", "(왠지 사기꾼 같은데...)");
                Nylon_TryInvestBbang("네, 만 냥 투자할게요.", "ID_F_CHICKEN_WHAT_INVEST", "ID_F_NOMONEY_BEFORECOIN");
            ...
            case "ID_F_CHICKEN_WHAT_NOTINVEST":
                ...
                Nylon_TryInvestBbang("정말요? 그럼 당장 투자할게요.", "ID_F_CHICKEN_WHAT_NOTINVEST_INVEST",
            ...
            case "ID_F_CHICKEN_NOTINVEST":
                ...
                pmtComtrol.AddString("나이롱마스크", "저한테 만 냥을 투자하면 포켓볼 뽱을 세 개까지 가져올 수 있어요우~");
            ...
            case "ID_F_WHAT":
                ...
                pmtComtrol.AddString("나이롱마스크", "포켓볼 뽱 투자에요우~ 만 냥 투자로 최대 세 개까지 얻을 수 있어요우~");
                ...
                Nylon_TryInvestBbang("좋아요. 만 냥 투자할게요.", "ID_F_WHAT_INVEST", "ID_F_NOMONEY_BEFORECOIN");
            ...
            case "ID_F_SECOND_IFINVESTED_BREADS_3":
                ...
                Nylon_TryInvestBbang("당연하죠! 당장 투자할래요.", "ID_F_SECOND_IFINVESTED_3BREADS_INVEST",
            ...
            case "ID_F_SECOND_IFINVESTED_3BREADS_INVEST":
                ...
                pmtComtrol.AddString("나이롱마스크", "만 냥 잘 받았어요우~ 대박의 기운이 가득한 이몸만 믿으세요우~");
                playerHealthManager.UpdateMoney(-10000);
                PlayerPrefs.SetInt("NylonInvested", 1);
            ...
            case "ID_F_SECOND_IFINVESTED_BREADS_2":
                ...
                Nylon_TryInvestBbang("좋아요!", "ID_F_SECOND_IFINVESTED_2BREADS_INVEST",
                    "ID_F_NOMONEY_AFTERECOIN_BBANGLAST");
            ...
            case "ID_F_SECOND_IFINVESTED_2BREADS_INVEST":
                ...
                pmtComtrol.AddString("나이롱마스크", "만 냥 잘 받았어요우~ 제가 알아서 잘 투자해드릴테니 다음에 봐요우~");
                playerHealthManager.UpdateMoney(-10000);
                PlayerPrefs.SetInt("NylonInvested", 1);
            ...
            case "ID_F_SECOND_IFINVESTED_BREADS_1":
                ...
                Nylon_TryInvestBbang("한번만 더 믿어볼게요.", "ID_F_SECOND_IFINVESTED_1BREAD_INVEST",
                    "ID_F_NOMONEY_AFTERECOIN_BBANGLAST");
            ...
            case "ID_F_SECOND_IFINVESTED_1BREAD_INVEST":
                ...
                pmtComtrol.AddString("나이롱마스크", "만 냥 잘 받았어요우~ 이번엔 더 잘해 볼게요우~");
                playerHealthManager.UpdateMoney(-10000);
                PlayerPrefs.SetInt("NylonInvested", 1);
            ...
            case "ID_F_SECOND_IFINVESTED_1BREAD_INVEST_END":
                storeOutAction = "(만 냥이나 투자해서 빵을 한 개밖에 못 얻다니. 조금 아쉽지만 다음 결과를 기다려보자.)";
                pmtComtrol.AddNextAction("main", "store_out");
                break;
            ...
            case "ID_F_SECOND_IFINVESTED_BREADS_0":
                ...
                Nylon_TryInvestBbang("정말이죠...? 그럼 딱 한번만 더 투자해 볼게요.", "ID_F_SECOND_IFINVESTED_0BREADS_INVEST",
                    "ID_F_NOMONEY_AFTERECOIN_BBANGLAST");
            ...
            case "ID_F_SECOND_IFINVESTED_0BREADS_INVEST":
                ...
                pmtComtrol.AddString("나이롱마스크", "만 냥 잘 받았어요우~ 이번엔 진짜 잘 될 거니까 저만 믿으세요우~");
                playerHealthManager.UpdateMoney(-10000);
                PlayerPrefs.SetInt("NylonInvested", 1);
            ...
            case "ID_F_SECOND_IFINVESTED_0BREADS_INVEST_END":
                storeOutAction = "(만 냥이나 투자해서 빵을 하나도 못 얻다니 이거 완전 손해잖아? 다음 투자는 잘 되기를 기도한다.)";
                pmtComtrol.AddNextAction("main", "store_out");
                break;
            ...
            case "ID_F_SECOND_IFNOTINVESTED":
                pmtComtrol.AddString("나이롱마스크", "아뇽하세요우~ 파랑새 치킨임니다~");
                pmtComtrol.AddString("나이롱마스크", "포켓볼 뽱에 투자해볼 생각이 생기셨나요우~? 만 냥이면 포켓볼 뽱을 최대 3개 까지 얻을 수 있어요우~");
                Nylon_TryInvestBbang("네, 투자할게요.", "ID_F_SECOND_IFNOTINVESTED_INVEST",
                    "ID_F_NOMONEY_AFTERECOIN_BBANGLAST");
    ...
    public void Nylon_f(string ID)
    {
        ...
        switch (ID)
        {
            ...
            case "ID_F_SECOND_IFINVESTED_BREADS_3":
                ...
                pmtComtrol.AddString("훈이", "와 3개씩이나!");
                pmtComtrol.AddString("나이롱마스크", "이 정돈 기본이에요우~! 또 투자해 볼래요우~?");
                Nylon_TryInvestBbang("당연하죠! 당장 투자할래요.", "ID_F_SECOND_IFINVESTED_3BREADS_INVEST", "ID_F_NOMONEY_BREAD",
                    "Nylon_f");
            ...
            case "ID_F_SECOND_IFINVESTED_3BREADS_INVEST":
                pmtComtrol.AddString("나이롱마스크", "똑똑한 선택이네요우~");
                pmtComtrol.AddString("나이롱마스크", "만 냥 잘 받았어요우~ 이번에도 투자 잘해서 대봑 한번 터뜨려 볼게요우~");
            ...
            case "ID_F_SECOND_IFINVESTED_2BREADS_INVEST":
                pmtComtrol.AddString("나이롱마스크", "투자의 묘미를 알아주는 분이군요우~");
                pmtComtrol.AddString("나이롱마스크", "만 냥 잘 받았어요우~ 다음번엔 뽱을 더 많이 가져오도록 노력할게요우~");
                if (PlayerPrefs.HasKey("NylonDomiTutorial"))
            ...
            case "ID_F_SECOND_IFINVESTED_BREADS_1":
                ...
                Nylon_TryInvestBbang("한번만 더 믿어볼게요.", "ID_F_SECOND_IFINVESTED_1BREAD_INVEST", "ID_F_NOMONEY_BREAD",
                    "Nylon_f");
            ...
            case "ID_F_SECOND_IFINVESTED_1BREAD_INVEST":
                ...
                pmtComtrol.AddString("나이롱마스크", "만 냥 잘 받았어요우~ 다음번엔 확실히 뽱을 더 많이 가져올 수 있을 거에요우~");
                if (PlayerPrefs.HasKey("NylonDomiTutorial"))
            ...
            case "ID_F_SECOND_IFINVESTED_0BREADS_INVEST":
                ...
                pmtComtrol.AddString("나이롱마스크", "만 냥 잘 받았어요우~ 다음 번엔 절대 투자 실패 같은 거 안 할 거에요우~");
                if (PlayerPrefs.HasKey("NylonDomiTutorial"))
            ...
            case "ID_F_FRIEND_MAIN_BREAD":
                pmtComtrol.AddString("나이롱마스크", "포켓볼 뽱~에 투자하고 싶으시군요우~ 만 냥이면 포켓볼 뽱을 최대 3개 까지 얻을 수 있어요우~");
            ...
            case "ID_F_FRIEND_MAIN_COIN":
                ...
                var balancePercent = doMiCoinManager.GetBalancePercent();
                if (doMiCoinManager.GetAmount() > 0)
                {
            ...
            case "ID_F_FRIEND_MAIN_COIN_BUY":
                pmtComtrol.AddString("나이롱마스크", "지금 코인 가격은 " + doMiCoinManager.GetPrice() + "냥 이에요우~");
                Nylon_OpenTradePanel(true);
                if (doMiCoinManager.diff < 0.6)
                {
                ...
                else if (doMiCoinManager.diff < 0.7)
                {
                    pmtComtrol.AddString("나이롱마스크", "오늘이 블랙프라이데이인가요우~? 도미 코인이 미친듯한 할인을 하고 있네요우~");
                }
                else if (doMiCoinManager.diff < 0.85)
                {
                    pmtComtrol.AddString("나이롱마스크", "바닥을 다지는 중이네요우~ 저라면 추매했어요우~ 물을 탈 때에요우~");
                }
                else if (doMiCoinManager.diff < 1)
                {
                    pmtComtrol.AddString("나이롱마스크", "도미 코인이 손님을 낚아먹으려고 하는군요우~ 물론 오를수도 있고 내릴 수도 있죠우~");
                }
                else if (doMiCoinManager.diff < 1.15)
                {
                    pmtComtrol.AddString("나이롱마스크", "이건 오른 것도 아니에요우~ 상승세 한번 타면 오늘이 제일 싸다는 걸 알게 될 거에요우~");
                }
                else if (doMiCoinManager.diff < 1.4)
                {
                    pmtComtrol.AddString("나이롱마스크", "꽤 올랐네요우~ 하지만 아직 늦지 않았어요우~ 이 가격보다 오르기만 하면 이득이에요우~");
                }
                else if (doMiCoinManager.diff < 1.8)
                {
                    pmtComtrol.AddString("나이롱마스크", "이미 버스 떠나버렸네요우~ 다음 버스를 기다리는 게 어떨까요우~? 물론 다음 버스가 온다면 말이죠우~");
                }
                else
                {
            ...
            case "ID_F_FRIEND_MAIN_COIN_BUY_TRADE":
                Nylon_TradeCoin(TradeType.buy, "ID_F_FRIEND_MAIN_COIN_BUY_NUMBER", "ID_F_FRIEND_MAIN_COIN_BUY_NOTBUY",
                    "ID_F_NOMONEY_COIN");
                break;

            case "ID_F_FRIEND_MAIN_COIN_BUY_NUMBER":
                pmtComtrol.AddString("나이롱마스크",
                    "총 " + doMiCoinManager.BuyAmount * doMiCoinManager.GetPrice() + "냥이에요우~ 구매를 확정하시겠어요우~?");
            ...
            case "ID_F_FRIEND_MAIN_COIN_BUY_NUMBER_YES":
                doMiCoinManager.BuyCoin(doMiCoinManager.BuyAmount);
                pmtComtrol.AddString("나이롱마스크", "여기 도미 코인 " + doMiCoinManager.BuyAmount + "개에요우~ ");
                pmtComtrol.AddString("나이롱마스크", "이제 도미 코인을 " + doMiCoinManager.GetAmount() + "개 가지고 계시네요우~");
                pmtComtrol.AddString("나이롱마스크", "더 하고 싶은 일이 있으신가요우~?");
                pmtComtrol.AddNextAction("Nylon_f", "ID_F_FRIEND_MAIN_INTERSECTION");
                break;
            ...
            case "ID_F_FRIEND_MAIN_COIN_SELL":
                pmtComtrol.AddString("나이롱마스크", "지금 코인 가격은 " + doMiCoinManager.GetPrice() + " 이에요우~");
                Nylon_OpenTradePanel(true);
                if (doMiCoinManager.diff < 0.6)
                {
                ...
                {
                    pmtComtrol.AddString("나이롱마스크", "바닥 밑에 지하실이 있었네요우~ 당신이 선택한 코인이에요우~ 악으로 깡으로 버티세요우~!");
                }
                else if (doMiCoinManager.diff < 0.85)
                {
                    pmtComtrol.AddString("나이롱마스크", "마음이 아프네요우~ 하지만 아프니까 청춘이죠우~ 단골 손님은 이팔청춘이네요우~ 너무 아프니까요우~");
                }
                else if (doMiCoinManager.diff < 1)
                {
                    pmtComtrol.AddString("나이롱마스크", "약간의 손실이 있네요우~ 하지만 이정도로 엄살 부리는 찌질이는 없겠죠우~?");
                }
                else if (doMiCoinManager.diff < 1.15)
                {
                    pmtComtrol.AddString("나이롱마스크", "오늘 장은 심심하네요우~ 차라리 적금을 드는 게 낫겠어요우~");
                }
                else if (doMiCoinManager.diff < 1.4)
                {
                    pmtComtrol.AddString("나이롱마스크", "나쁘지 않은 수익이에요우~ 이 정도는 돼야 코인할 맛이 나죠우~");
                }
                else if (doMiCoinManager.diff < 1.8)
                {
                    pmtComtrol.AddString("나이롱마스크", "가즈아~ 불꽃 같은 상승에 제 마음이 뜨거워지는군요우~");
                }
                else
                {
                ...
                pmtComtrol.AddString("나이롱마스크",
                    "지금 도미 코인을 " + doMiCoinManager.GetAmount() + "개 가지고 있으시네요우~ 몇 개 파시겠어요우~? ");
                pmtComtrol.AddNextAction("Nylon_f", "ID_F_FRIEND_MAIN_COIN_SELL_TRADE");
                break;

            case "ID_F_FRIEND_MAIN_COIN_SELL_TRADE":
                Nylon_TradeCoin(TradeType.sell, "ID_F_FRIEND_MAIN_COIN_SELL_NUMBER",
                    "ID_F_FRIEND_MAIN_COIN_SELL_NOTSELL", "");
                break;

            case "ID_F_FRIEND_MAIN_COIN_SELL_NUMBER":
                pmtComtrol.AddString("나이롱마스크",
                    "총 " + doMiCoinManager.SellAmount * doMiCoinManager.GetPrice() + "냥을 얻을 수 있어요우~ 판매를 확정하시겠어요우~?");
            ...
            case "ID_F_FRIEND_MAIN_COIN_SELL_YES":
                doMiCoinManager.SellCoin(doMiCoinManager.SellAmount);
                pmtComtrol.AddString("나이롱마스크", "도미 코인을 " + doMiCoinManager.SellAmount + "개 팔았어요우~ ");
                pmtComtrol.AddString("나이롱마스크", "이제 도미 코인이 " + doMiCoinManager.GetAmount() + "개 남았네요우~");
                pmtComtrol.AddString("나이롱마스크", "더 하고 싶은 일이 있으신가요우~?");
                pmtComtrol.AddNextAction("Nylon_f", "ID_F_FRIEND_MAIN_INTERSECTION");
                break;
            ...
            case "ID_F_FRIEND_COINEXPLAIN":
                ...
                pmtComtrol.AddString("훈이", "이것도 만 냥씩 투자하면 되나요?");
            ...
            case "ID_F_FRIEND_COINEXPLAIN_INVEST_2":
                pmtComtrol.AddString("나이롱마스크", "지금 코인 가격은 " + doMiCoinManager.GetPrice() + "냥 이에요우~ 코인을 몇 개 구매할래요우~?");
                Nylon_OpenTradePanel(true);
                pmtComtrol.AddNextAction("Nylon_f", "ID_F_FRIEND_COINEXPLAIN_INVEST_2_TRADE");
                break;

            case "ID_F_FRIEND_COINEXPLAIN_INVEST_2_TRADE":
                Nylon_TradeCoin(TradeType.buy, "ID_F_FRIEND_COINEXPLAIN_INVEST_NUMBER",
                    "ID_F_FRIEND_COINEXPLAIN_INVEST_NOTINVEST", "ID_F_NOMONEY_COIN");
                break;

            case "ID_F_FRIEND_COINEXPLAIN_INVEST_NUMBER":
                pmtComtrol.AddString("나이롱마스크",
                    "총 " + doMiCoinManager.GetPrice() * doMiCoinManager.BuyAmount + "냥이에요우~ 구매를 확정하시겠어요우~?");
            ...
            case "ID_F_FRIEND_COINEXPLAIN_INVEST_YES":
                pmtComtrol.AddString("나이롱마스크",
                    "여기 도미 코인 " + doMiCoinManager.BuyAmount + "개에요우~ 역시 손님은 도전 정신이 투철하시군요우~ 도전하는 사람은 아름다워요우~");
                Nylon_OpenTradePanel(false);
    ...
    public void JangJokBal(string ID)
    {
        ...
        switch (ID)
        {
            ...
            case "ID_J_GAME":
                ...
                pmtComtrol.AddString("장왕", "그리고 너 각오해, 딱 기다려. 하는 마음으로 손님 입에 던지면 돼요.");
            ...
            case "ID_J_GAME_EXPLAIN":
                ...
                pmtComtrol.AddString("장왕", "손님은 한 분만 오실 때도 있고요, 단체로 오실 때도 있어요.");
                // No command found on line 57 : 
                pmtComtrol.AddString("장왕", "그런데 손님 입에 못 넣으면 점수 까버릴 거에요.");
                pmtComtrol.AddString("장왕", "대신 연속으로 성공하면 콤보 점수를 드릴게요.");
            ...
            case "ID_J_GAME_OVER_GOOD":
                ...
                {
                    pmtComtrol.AddString("장왕", "알밥 천국에도 등록해 드렸으니까, 이제부터는 원할 때 언제든지 오실 수  있어요.");
    ...
    public void Debug_gotoBbobgi()
    {
        ...
        parkBtn.SetActive(false);
        GetComponent<PlayerHealthManager>().SetHeart(-1);
        front_panel.SetActive(true);
        frontCha.GetComponent<Animator>().SetTrigger("walk");
        main_panel.GetComponent<Animator>().SetTrigger("hide");
        lower_bar.GetComponent<Animator>().SetTrigger("hide");
    ...
    public void GotoCu()
    {
        if (currentLocation == "home")
        {
            if (playerHealthManager.ConsumeSingleHeart())
            {
                ...
                audioController.PlayMusic(1);
                GoToStore(0);
                dangunNotificationManager.HideNoticePanel();
            }
            else
            {
                balloonUIManager.ShowMsg("지금은 피곤하다.. \n 너튜브나 보면서 쉴까..");
        ...
        {
            balloonUIManager.ShowMsg("여기서는 이동할 수 없어..\n집으로 돌아갈까?");
        }
    }

    public void Debug_reloadBbobgi()
    {
        newStore.GetComponent<StoreControl>().bbobgiPanel.bbobgiController.StartGame();
    }
    ...
    {
        AddBBangType(1, "DEBUG", "maple");
    }

    public void Debug_addPurin()
    {
        AddBBangType(1, "DEBUG", "purin");
    }
