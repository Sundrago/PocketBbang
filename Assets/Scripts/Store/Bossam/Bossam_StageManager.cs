...
public class Bossam_StageManager : MonoBehaviour
{
    ...
    public void AnimIn()
    {
        ...
        var charidx = new int[3];
        charidx[0] = Random.Range(0, 3);
        charidx[1] = Random.Range(0, 3);
        charidx[2] = Random.Range(0, 3);

        while (charidx[1] == charidx[0]) charidx[1] = Random.Range(0, 3);
        while (charidx[2] == charidx[0] || charidx[2] == charidx[1]) charidx[2] = Random.Range(0, 3);
        ...
        foreach (var character in chardata) character.character.CloseMouth();
        ...
        {
            var newChar = Instantiate(char_prefabs[charIdx], gameObject.transform);
    ...
    public void AnimOut()
    {
        gameObject.transform.DOLocalMoveX(-1000, 0.5f).SetEase(Ease.OutExpo).OnComplete(() => { Destroy(gameObject); });
    }
