...
public class StoreDiamondWatchAdsPanel : MonoBehaviour
{
    ...
    private StoreDiamondAdsData diamondAdsData;
    private readonly int[] rewardAmt = { 2, 3, 4, 5, 5 };

    [Button]
    public void UpateUI()
    {
        if (diamondAdsData == null) LoadData();
        var count = diamondAdsData.count;
    ...
    {
        ADManager.Instance.PlayAds(ADManager.AdsType.diamondStore);
        // WatchedAd();
    }
    ...
    {
        ClosePanel();
        RewardItemManager.Instance.Init(new[] { 1003 }, new[] { amt }, "DiamondAd", "광고를 보고 보상을 받았다!");
    ...
    private void LoadData()
    {
        if (PlayerPrefs.HasKey("StoreDiamondAdsData"))
        {
            ...
            var date = Converter.StringToDateTime(diamondAdsData.date);
            if (date.DayOfYear != DateTime.Now.DayOfYear)
            {
                diamondAdsData.date = Converter.DateTimeToString(DateTime.Now);
        ...
        else
        {
            ...
            diamondAdsData.date = Converter.DateTimeToString(DateTime.Now.AddDays(-1));
            diamondAdsData.count = 0;
