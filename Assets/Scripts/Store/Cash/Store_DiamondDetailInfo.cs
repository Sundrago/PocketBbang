...
public class Store_DiamondDetailInfo : MonoBehaviour
{
    ...
    public void Init(int _idx, int amount, int price, bool isBonus)
    {
        ...
        descr_ui.text = "다이아몬드\n" + Converter.IntToCommaSeparatedString(amount) + "개를 구매할까요?";

        amt0.text = Converter.IntToCommaSeparatedString(amount) + "개";
        amt1.text = Converter.IntToCommaSeparatedString(amount) + "개";
    ...
    private IEnumerator LoadRewardItems(int amount)
    {
        ...
        DOVirtual.Int(amount, amount * 2, 2f,
                x => { descr_ui.text = "다이아몬드\n" + Converter.IntToCommaSeparatedString(x) + "개를 구매할까요?"; })
