...
public class Store_DiamondManager : MonoBehaviour
{
    ...
    public void DiamondPurchased(int idx)
    {
        ...
        if (boughtData.boughtCount[idx] == 0)
        {
            ...
            amts = new int[2] { JsonData.Instance.StoreDiamondDatas[idx].amount, JsonData.Instance.StoreDiamondDatas[idx].amount};
        }
        else
        {
            idxs = new int[1] { 1003 };
    ...
    public void LoadDiamondBoughtData()
    {
        ...
        for (int i = 1; i < 6; i++)
        {
            storeItems[i].SetFirstBuy(boughtData.boughtCount[i] == 0) ;
        }
    }
